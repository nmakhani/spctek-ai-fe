...
export function validateEstimatorForm(values: FormValues): FormErrors {
  ...
  if (!values.performanceNotification?.trim()) {
    errors.performanceNotification =
      "Please paste your Amazon performance notification";
  }
  ...
  // if (
  //   !values.businessModel?.trim() ||
  //   values.businessModel === "- Select Business Model -"
  // ) {
  //   errors.businessModel = "Please select your business model";
  // }

  // if (!values.suspensionCause?.trim()) {
  //   errors.suspensionCause =
  //     "Please describe what you believe caused the suspension";
  // }
