...
const initialMessage: ChatMessage = {
	...
	role: 'assistant',
	content:
		"Hi, I'm NOVA. Ask me about SPCTEK.AI services, automation ideas, local AI, or which path fits your business. I keep things short unless you invite me to nerd out.",
};

const storedMessagesKey = 'spctek-nova-message-pairs';
...
export default function NovaChatWidget() {
	...
	const sessionId = useMemo(() => {
		if (typeof window === 'undefined') return 'nova-session';

		const storageKey = 'spctek-nova-session-id';
	...
	const submitMessage = async (value?: string) => {
		...
		try {
			...
			if (!response.ok) {
				...
				throw new Error(data?.error || 'NOVA could not respond right now.');
			}

			if (!response.body) {
				throw new Error('NOVA returned an empty stream.');
			}
			...
			const message =
				requestError instanceof Error
					...
					: 'NOVA is temporarily unavailable. Please try again soon.';
			setError(message);
			setMessages((current) => [
		...
		<AnimatePresence mode="wait">
			{isOpen ? (
				<motion.section
					...
					aria-label="NOVA SPCTEK AI chat"
				>
						...
						<div className="relative flex items-center justify-between gap-3">
							<div className="flex min-w-0 items-center gap-3">
								...
								<div className="min-w-0">
									<div className="flex items-center gap-2">
										<h2 className="truncate font-heading text-xl font-bold leading-none">NOVA</h2>
							...
							<div className="flex shrink-0 items-center gap-2">
								<button
									...
									aria-label={isFullPage ? 'Return NOVA chat to compact view' : 'Open NOVA chat in full page view'}
					...
					<form onSubmit={handleSubmit} className="border-t border-white/10 bg-black/20 p-3">
						<div className="border-white/12 flex items-end gap-2 rounded-2xl border bg-white/[0.06] p-2 focus-within:border-[#8c96ff] focus-within:ring-2 focus-within:ring-[#606bfa]/35">
							<textarea
								...
								placeholder="Ask NOVA about SPCTEK.AI..."
								rows={1}
