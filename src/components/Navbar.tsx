		...
		<header className="fixed left-0 right-0 top-0 z-50">
			...
			>
				...
				<div className="relative z-10 flex h-full w-full items-center justify-between">
					...
					<nav className="ml-auto mr-8 hidden items-center gap-8 md:flex">
						{navLinks.map((link) => (
							<Link
								key={link.name}
								href={link.href}
								className="font-poppins text-sm font-light text-white transition-colors hover:text-brand-secondary"
							>
								{link.name}
							</Link>
						))}
