  ...
  const renderField = (field: FieldConfig) => {
    ...
    return (
      <div key={field.name} className={spanClass}>
          ...
          <div style={{ overflow: "hidden" }}>
            {field.type === "action" ? (
              <button
                ...
                className="flex w-full items-center justify-between px-5 py-4 transition-all hover:bg-white/5 active:bg-white/10 text-lg font-medium"
              >
        ...
        <div className="mt-2 ml-4 flex justify-between items-start px-1">
          <div>
          ...
          <div className="relative z-10 flex gap-4 pt-1">
            ...
            {clearable && (
              <button
                ...
                onClick={handleClear}
                disabled={loading}
                className="flex-1 min-w-[120px] rounded-2xl border border-white py-3.5 text-lg font-semibold text-white transition-all hover:bg-white/10 disabled:opacity-50"
              >
                Clear Form
              </button>
            )}
