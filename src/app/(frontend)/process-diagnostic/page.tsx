	...
	return (
		...
		>
			<div className="flex items-start gap-3">
				...
				>
					{selected && <div className="bg-cyan h-2 w-2 rounded-full" />}
				</div>
				<div>
	...
	return (
		<div className="mb-8">
			<div className="mb-2 flex items-center justify-between">
				...
				<span className="text-cyan text-xs font-semibold">{Math.round(pct)}%</span>
			</div>
...
export default function ProcessDiagnosticPage() {
	...
	const set = (name: keyof FormData, value: string) => setForm((p) => ({ ...p, [name]: value }));
						...
						<AnimatePresence mode="wait">
								...
								<StepWrapper key="loading">
									<div className="py-12 text-center">
										<div className="relative mx-auto mb-8 h-16 w-16">
											<div className="border-cyan/20 absolute inset-0 rounded-full border-2" />
											<div className="border-cyan absolute inset-0 animate-spin rounded-full border-t-2" />
											<div className="border-purple absolute inset-[6px] animate-spin rounded-full border-t-2 [animation-direction:reverse] [animation-duration:1.5s]" />
										</div>
							...
							{phase === 'results' && (
								<StepWrapper key="results">
									{/* Score header */}
									<div className="mb-8 flex items-center gap-6 border-b border-white/[0.06] pb-8">
										<div
											className={`relative flex h-24 w-24 flex-shrink-0 items-center justify-center rounded-full border-2 ${category.border}`}
										...
										<div>
											...
											<p className={`text-2xl font-black ${category.color} mb-1`}>
												{category.label}
											</p>
											<p className="text-sm leading-relaxed text-slate-400">{category.desc}</p>
										</div>
									</div>
									...
									<div className="mb-8 space-y-4">
										{pointers.map((p, i) => (
											<motion.div
												...
												className={`rounded-xl border border-l-4 border-white/[0.06] bg-white/[0.02] p-4 ${p.accent}`}
											>
												<h4 className="mb-1 text-sm font-bold text-white">{p.title}</h4>
												<p className="mb-2 text-sm leading-relaxed text-slate-400">{p.body}</p>
									...
									{/* CTA box */}
									<div className="border-cyan/20 bg-cyan/[0.04] rounded-2xl border p-6">
										<p className="mb-1 text-sm font-bold text-white">
											Ready to fix these in 30 days?
										</p>
										<p className="mb-4 text-xs leading-relaxed text-slate-400">
											We&apos;ll map your entire process landscape and identify which automations
											give you the fastest payback — in a single 30-min session.
										</p>
										<Link
											href="/#contact"
											className="glow-btn w-full justify-center !px-6 !py-2.5 !text-sm"
										>
								...
								<StepWrapper key={`step-${step}`}>
									...
									{step === 1 && (
										<div className="space-y-6">
											<div>
												<h2 className="mb-1 text-xl font-black text-white">Operational Baseline</h2>
											...
											<div>
												<label className="form-label">What is your primary goal right now?</label>
												<select
													value={form.motive}
											...
											<div className="pt-2">
												<button onClick={() => next(2)} className="glow-btn w-full justify-center">
									...
									{step === 2 && (
										<div className="space-y-6">
											...
											<div className="flex gap-3 pt-2">
												...
												<button onClick={() => next(3)} className="glow-btn flex-1 justify-center">
									...
									{step === 3 && (
										<div className="space-y-6">
											...
											<div>
												<label className="form-label">The Founder Bottleneck</label>
												<div className="space-y-2">
													{[
														{
															v: 'Everything',
											...
											<div>
												<label className="form-label">How is customer communication managed?</label>
												<div className="space-y-2">
													{[
														{
															v: 'Manual',
											...
											<div className="flex gap-3 pt-2">
												...
												<button onClick={() => next(4)} className="glow-btn flex-1 justify-center">
									...
									{step === 4 && (
										<div className="space-y-6">
											<div>
												<h2 className="mb-1 text-xl font-black text-white">The Broken Link</h2>
												<p className="text-sm text-slate-400">
											...
											<div>
												<label className="form-label">Have you tried to fix this before?</label>
												<div className="space-y-2">
													{[
														{
															v: 'Never',
											...
											<div className="flex gap-3 pt-2">
												...
												<button onClick={() => next(5)} className="glow-btn flex-1 justify-center">
													Get My Scorecard →
												</button>
											</div>
										</div>
									...
									{step === 5 && (
										<div className="space-y-6">
											<div>
												<h2 className="mb-1 text-xl font-black text-white">Almost There</h2>
												<p className="text-sm text-slate-400">
													Enter your details to unlock your Process Health Score and personalized
													automation playbook.
												</p>
											</div>
											...
											<div className="flex items-center gap-4 rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
												<div className="border-cyan/40 bg-cyan/[0.06] flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full border-2">
													<div className="text-cyan select-none text-lg font-black blur-[3px]">
														{score}
													</div>
												</div>
												<div>
											...
											{submitError && <p className="text-rose text-sm">{submitError}</p>}

											<p className="text-xs leading-relaxed text-slate-600">
												We take privacy seriously — your details are only used to send your
												scorecard and optional follow-up. No spam, ever.
											</p>
